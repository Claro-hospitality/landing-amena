...
Standalone public marketing site for **Amena** (a restaurant with a companion B2B service called Social Kitchen). It's a read-only, single-purpose site with no backend, no auth, and no client-side routing library. It was extracted from the `/conoce-amena` route of a larger private monorepo (`amena-frontend`); shared pieces it depended on (`cn`, `LogotipoAmena`, the `theme.css` tokens) were inlined here so this repo has zero dependency on that monorepo.
...
- **El administrador vive en otro repo**: el panel `/admin/*` se movió al proyecto standalone `amena-admin` (carpeta hermana `../amena-admin`, site de Firebase Hosting `amena-admin` en el mismo proyecto `amena-20df0`). Los módulos compartidos (`lib/supabase`, `lib/utils`, `lib/qr`, `lib/fechas`, `lib/sinergypay`, `data/eventos`, `data/reservaciones`, `theme.css`) están **duplicados** allá: si cambias el esquema de Supabase o esos módulos, replica el cambio en los dos repos. Las migraciones y edge functions (`supabase/`) siguen siendo de este repo.
